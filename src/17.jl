...
function tetrisheight(io; n=2022)
...
function findcycle(io)

    n = 4000

    while true
        _, cyc = tetrisheight(io, n=n)
...
function bigtetrisheight(io)
    starti, cyc = findcycle(io)

    starth, _ = tetrisheight(io, n=starti - 1)
...
function solutions(io::String=joinpath(@__DIR__, "..", "data", "17.txt"))
    partone, _ = ispath(io) ? tetrisheight(io) : tetrisheight(IOBuffer(io))
    parttwo = ispath(io) ? bigtetrisheight(io) : bigtetrisheight(IOBuffer(io))
    partone, parttwo
end
